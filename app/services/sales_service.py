...
from app.models.schema import (
    Sale, BillReceipt, User, ReceiptStatus,
    Shift, ShiftStatus, PaymentMethod,
)
...
class SalesService:
    ...
    # ------------------------------------------------------------------
    # Read helpers — all return (records, total_count)
    # ------------------------------------------------------------------

    def get_receipts_ledger(self, search_query: str = None, page: int = 1):
        return self.sales_repo.get_all_receipts(search_query, page)

    def get_receipt_details(self, receipt_id: int):
        return self.sales_repo.get_receipt_by_id(receipt_id)

    def get_flat_sales_history(self, search_query: str = None, page: int = 1):
        return self.sales_repo.get_all_sales_standalone(search_query, page)

    def get_payments_ledger(self, page: int = 1):
        return self.sales_repo.get_all_payments(page)
    ...
    def checkout_cart(
        ...
        status_enum = (
            ReceiptStatus.COMPLETED if p_amount >= total_accumulation else ReceiptStatus.PENDING
        )
